...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (neighbors.isEmpty) Stream.Empty
      ...
      val res = for {
        n <- initial
        n2 <- newNeighborsOnly(neighborsWithHistory(n._1,n._2), explored + n._1)
      } yield from(n2 #:: initial,explored + n2._1)
   ...
   */
  lazy val pathsToGoal: Stream[(Block, List[Move])] = pathsFromStart.filter( p => p._1.isStanding && p._1.b1 == goal )
   ...
   * position.
   */
  lazy val solution: List[Move] = pathsToGoal.sortBy(x => x._2.length).head._2.reverse
}
